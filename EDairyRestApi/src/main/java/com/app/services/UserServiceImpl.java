...
public class UserServiceImpl implements UserService {
	...
	public LoggedInUserDto getUserByUsername(String username) {
		System.out.println(" in user im[plementatipon " + username);
		...
		System.out.println("in login getuserbyname method after save login "+login);
		...
		if(username.equals("admin1") || username.equals("admin2")) {
			...
			System.out.println(" in login else method "+login);
			User user =  userRepo.findByLogin(login).orElseThrow(() -> new RuntimeException("User Not Found"));
			System.out.println(" after save user from else "+user);
			loggedInUserDto = mapper.map(user, LoggedInUserDto.class);
			System.out.println("loggedInUserDto"+loggedInUserDto);
			loggedInUserDto.setUsername(username);
...
//		throw new ResourceNotFoundException("Invalid Note Id : Updation Failed");
	...
	@Override
	public LoggedInUserDto getuser(SigninDto dto) {
		System.out.println(" in user im[plementatipon " + dto.getUsername());
		System.out.println(loginRepo.findByUsername(dto.getUsername()));
		Login login = loginRepo.findByUsername(dto.getUsername()).orElseThrow(() -> new RuntimeException("User Not Found"));
//	       login = loginRepo.getPassword().equals(dto.getPassword())
//	    		   .orElseThrow(() -> new RuntimeException("password is wrong Found"));
		System.out.println("in login getuserbyname method after save login "+login);
		LoggedInUserDto loggedInUserDto;
		if(dto.getUsername().equals("admin1") ||dto.getUsername().equals("admin2")) {
			Admin admin = adminRepo.findByLogin(login).orElseThrow(() -> new RuntimeException("User Not Found"));
			loggedInUserDto = mapper.map(admin, LoggedInUserDto.class);
			loggedInUserDto.setUsername(dto.getUsername());
			loggedInUserDto.setRole("ROLE_ADMIN");	
			return loggedInUserDto;
		}
		else {
			...
			User user =  userRepo.findByLogin(login).orElseThrow(() -> new RuntimeException("User Not Found"));
		 //    if( user.loginRepo.getPassword().equals(dto.getPassword()))
			if( user.getLogin().getPassword().equals(dto.getPassword())) 
		   {
